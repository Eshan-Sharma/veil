...
```
LendingPool     seeds: ["pool",         token_mint]   program: Veil
UserPosition    seeds: ["user_position", user, pool]  program: Veil
EncryptedPos    seeds: ["enc_pos",       user, pool]  program: Veil
IkaDwalletPos   seeds: ["ika_pos",       pool, user]  program: Veil
PoolAuthority   seeds: ["authority",     pool]         program: Veil
CPI Authority   seeds: ["__ika_cpi_authority"]         program: Veil
```
...
```
0x00  Initialize         0x08  EnablePrivacy      0x11  IkaRegister
0x01  Deposit            0x09  PrivateDeposit     0x12  IkaRelease
0x02  Withdraw           0x0A  PrivateBorrow      0x13  IkaSign
0x03  Borrow             0x0B  PrivateRepay       0x14  UpdateOraclePrice
0x04  Repay              0x0C  PrivateWithdraw    0x0D  UpdatePool
0x05  Liquidate          0x0D  UpdatePool         0x0E  PausePool
0x06  FlashBorrow        0x0E  PausePool          0x0F  ResumePool
0x07  FlashRepay         0x0F  ResumePool         0x10  CollectFees
```
...
<Callout type="info">
  The Encrypt program CPI is stubbed pending an SDK update from pinocchio 0.10 → 0.11. All five private instructions compile and route correctly; the graph execution call activates when the SDK ships the update.
</Callout>
