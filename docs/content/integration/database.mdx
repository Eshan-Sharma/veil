...
[Neon Postgres](https://neon.tech). Six tables, all defined in
...
CREATE INDEX idx_pools_mint      ON pools(token_mint);
CREATE INDEX idx_pools_authority ON pools(authority);
```
...
CREATE INDEX idx_positions_health ON positions(health_factor_wad)
...
`useVeilActions` after `confirmTransaction` resolves. The `signature`
column is `UNIQUE`, so callers can safely `POST` the same signature with
an updated status (e.g. `pending` → `confirmed`) without creating a
duplicate row — the endpoint uses `ON CONFLICT DO UPDATE`.
...
CREATE INDEX idx_tx_log_wallet ON tx_log(wallet, created_at DESC);
CREATE INDEX idx_tx_log_pool   ON tx_log(pool_address, created_at DESC);
```
