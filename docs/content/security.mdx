...
Veil uses a flag-based re-entrancy guard (`pool.flash_loan_active`) that is set during `FlashBorrow` and cleared on `FlashRepay`. Any instruction that mutates pool state while `flash_loan_active == true` returns an error.
...
`PausePool` halts all user-facing instructions (Deposit, Withdraw, Borrow, Repay, Liquidate, Flash Loan). Only the pool authority (set at `Initialize`) can pause or resume.
