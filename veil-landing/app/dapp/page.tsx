...
function getPoolIcon(symbol: string): string {
  const map: Record<string, string> = { SOL: "◎", BTC: "₿", ETH: "Ξ", XAU: "◈", USDC: "$" };
  return map[symbol.toUpperCase()] ?? "●";
}

function getPoolColor(symbol: string): string {
  const map: Record<string, string> = { SOL: "#7c3aed", BTC: "#f97316", ETH: "#6366f1", XAU: "#ca8a04", USDC: "#2563eb" };
...
function borrowRate(baseRate: number, slope1: number, slope2: number, optimalUtil: number, u: number): number {
  if (u <= optimalUtil) return baseRate + (slope1 * u) / optimalUtil;
  return baseRate + slope1 + (slope2 * (u - optimalUtil)) / (100 - optimalUtil);
}

function supplyRate(baseRate: number, slope1: number, slope2: number, optimalUtil: number, reserveFactor: number, u: number): number {
...
function AssetIcon({ pool, size = 34 }: { pool: PoolView; size?: number }) {
  ...
  const bg =
    type === "ika"   ? "linear-gradient(135deg,#f97316,#eab308)"
      ...
      <div className="glass-card" style={{ borderRadius: 18, overflow: "hidden" }}>
        ...
        <div style={{ padding: "14px 18px", borderBottom: "1px solid #f0f0f3" }}>
          ...
          <div style={{ height: 6, background: "#f0f0f3", borderRadius: 3, overflow: "hidden", marginBottom: 10 }}>
            <div style={{ height: "100%", width: enc ? "0" : `${util}%`, background: util > 80 ? "#dc2626" : util > 60 ? "#d97706" : "linear-gradient(90deg,#059669,#10b981)", borderRadius: 3, transition: "width .6s" }} />
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
            <div style={{ background: "#f9f9fb", borderRadius: 8, padding: "8px 10px", border: "1px solid #f0f0f3" }}>
              ...
              <div style={{ fontSize: 14, fontWeight: 700 }}>{enc ? <CipherVal seed={0} mask="$◉◉◉,◉◉◉" /> : formatBigAmount(pool.totalDeposits)}</div>
            </div>
            <div style={{ background: "#f9f9fb", borderRadius: 8, padding: "8px 10px", border: "1px solid #f0f0f3" }}>
              <div style={{ fontSize: 10, color: "#9ca3af", fontWeight: 500, marginBottom: 2 }}>Available</div>
              <div style={{ fontSize: 14, fontWeight: 700, color: "#059669" }}>{enc ? "••••" : formatBigAmount(availLiq > 0n ? availLiq : 0n)}</div>
            </div>
          </div>
        </div>
        ...
        <div style={{ padding: "14px 18px", borderBottom: "1px solid #f0f0f3" }}>
          ...
          <div style={{ display: "flex", gap: 14, marginTop: 8 }}>
            <span style={{ fontSize: 10.5, color: "#d97706", display: "flex", alignItems: "center", gap: 4 }}>
              <span style={{ width: 14, height: 2, background: "#d97706", display: "inline-block", borderRadius: 1 }} /> Borrow APY
            </span>
            <span style={{ fontSize: 10.5, color: "#059669", display: "flex", alignItems: "center", gap: 4 }}>
              <span style={{ width: 14, height: 2, background: "#059669", display: "inline-block", borderRadius: 1, opacity: 0.65 }} /> Supply APY
            </span>
            <span style={{ fontSize: 10.5, color: "#8b5cf6", display: "flex", alignItems: "center", gap: 4 }}>
              <span style={{ width: 10, height: 2, background: "#8b5cf6", display: "inline-block", borderRadius: 1, opacity: 0.6 }} /> Kink
            </span>
          </div>
        ...
        <div style={{ padding: "14px 18px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 14 }}>
            {[
              ...
              { k: "Liq. threshold", v: `${liqTh}%`,    info: "Health factor liquidation trigger" },
              { k: "Liq. bonus",     v: `+${liqBonus}%`, info: "Discount liquidators receive" },
              { k: "Reserve factor", v: `${rf}%`,        info: "Share of interest to protocol" },
            ].map((r, i) => (
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button
              ...
              style={{ flex: 1, padding: "10px", borderRadius: 12, fontSize: 14, fontWeight: 700, background: type === "ika" ? "linear-gradient(135deg,#f97316,#eab308)" : undefined }}
              onClick={() => setModal({ type: type === "ika" ? "ika-setup" : "supply", pool })}
            >
...
function PositionSummary({ connected, fhe, pools, positions, setModal }: {
  connected: boolean; fhe: boolean; pools: PoolView[]; positions: PositionRow[];
  setModal: (m: ModalState | null) => void;
}) {
  const hasPositions = connected && positions.length > 0;
  const totalDepositShares = positions.reduce((s, p) => s + BigInt(p.deposit_shares || "0"), 0n);
  const totalBorrowPrincipal = positions.reduce((s, p) => s + BigInt(p.borrow_principal || "0"), 0n);

  const metrics = [
    { label: "Deposit Shares",     value: hasPositions ? formatBigAmount(totalDepositShares) : "0",    color: connected ? "#0b0b10" : "#c4c4cc" },
    { label: "Borrow Principal",   value: hasPositions ? formatBigAmount(totalBorrowPrincipal) : "0",  color: connected ? "#dc2626" : "#c4c4cc" },
    { label: "Positions",          value: connected ? String(positions.length) : "0",                  color: connected ? "#0b0b10" : "#c4c4cc" },
    { label: "Pools Available",    value: String(pools.length),                                        color: connected ? "#059669" : "#c4c4cc" },
  ];
    ...
    <div style={{ position: "sticky", top: 82, display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      <div className="glass-card" style={{ borderRadius: 18, padding: 18 }}>
        ...
        {!hasPositions ? (
          ...
          <>
            {positions.filter(p => BigInt(p.deposit_shares || "0") > 0n).map((pos, i) => {
              const pool = pools.find(p => p.poolAddress.toBase58() === pos.pool_address);
              if (!pool) return null;
              const enc = fhe && getPoolType(pool.symbol) === "enc";
              return (
                <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 0", borderBottom: i < positions.length - 1 ? "1px solid #f7f7f9" : "none" }}>
                  <AssetIcon pool={pool} size={30} />
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
                      <span style={{ fontSize: 13, fontWeight: 600 }}>{pool.symbol}</span>
                      <span style={{ fontSize: 13, fontWeight: 600 }}>
                        {enc ? <CipherVal seed={20 + i} mask="◉◉◉◉" /> : formatBigAmount(BigInt(pos.deposit_shares))}
                      </span>
                    </div>
                    <div style={{ fontSize: 11.5, color: "#5b5b66" }}>
                      shares · HF {pos.health_factor_wad ? (Number(BigInt(pos.health_factor_wad) * 100n / WAD) / 100).toFixed(2) : "—"}
                    </div>
                  </div>
                </div>
              );
...
function MarketsView({ fhe, connected, setModal, pools, poolsLoading, poolsError, refreshPools, positions }: {
  fhe: boolean; connected: boolean;
  setModal: (m: ModalState | null) => void;
  pools: PoolView[]; poolsLoading: boolean; poolsError: string | null; refreshPools: () => void;
  ...
  return (
    <div className="fade-rise" style={{ maxWidth: 1280, margin: "0 auto", padding: "0 20px 40px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 10, marginBottom: 20 }}>
        <MetricCard label="Total Supplied"  value={formatBigAmount(totalDeposits)} sub="Protocol-wide" />
        <MetricCard label="Total Borrowed"  value={formatBigAmount(totalBorrows)} sub="Protocol-wide" />
        <MetricCard label="Utilization"     value={`${avgUtil.toFixed(0)}%`} color="#d97706" sub="Avg across pools" />
      ...
      <div style={{ display: "grid", gridTemplateColumns: selectedPool ? "1fr 360px" : "1fr 320px", gap: 16, alignItems: "start", transition: "grid-template-columns .25s" }}>
        <div>
          <div className="glass-card" style={{ borderRadius: 18, overflow: "hidden" }}>
            ...
            {poolsLoading ? (
              <div style={{ padding: "48px 18px", textAlign: "center", color: "#6b7280", fontSize: 14 }}>Loading pools from API...</div>
            ) : poolsError ? (
              ...
              <div style={{ padding: "48px 18px", textAlign: "center" }}>
                ...
                <div style={{ fontSize: 13, color: "#6b7280" }}>
                  An admin must create pools via <Link href="/dapp/admin" style={{ color: "#2563eb" }}>/dapp/admin</Link>.
                </div>
              </div>
            ) : pools.map((p, idx) => {
                ...
                >
                  ...
                  <div>
                    <div style={{ fontSize: 14, fontWeight: 500, color: "#0b0b10" }}>
                      {enc ? <CipherVal seed={idx * 2} mask="$◉◉◉,◉◉◉" /> : formatBigAmount(p.totalDeposits)}
                    </div>
                    <div style={{ fontSize: 11, color: "#9ca3af", marginTop: 2 }}>{enc ? "––" : formatBigAmount(availLiq > 0n ? availLiq : 0n) + " avail"}</div>
                  </div>
                  <div style={{ fontSize: 14, fontWeight: 700, color: "#059669" }}>+{sApy.toFixed(1)}%</div>
                  <div>
                    <div style={{ fontSize: 14, fontWeight: 500, color: "#0b0b10" }}>
                      {enc ? <CipherVal seed={idx * 2 + 1} mask="$◉◉◉,◉◉◉" /> : formatBigAmount(p.totalBorrows)}
                    </div>
                    <div style={{ fontSize: 11, color: "#9ca3af", marginTop: 2 }}>
                      {enc ? "––" : `${wadToPctNum(p.ltvWad)}% LTV`}
                    </div>
                  </div>
                  <div style={{ fontSize: 14, fontWeight: 700, color: "#d97706" }}>{bApy.toFixed(1)}%</div>
                  <div>
        ...
        <div style={{ position: "sticky", top: 82 }}>
          {selectedPool ? (
            ...
            <PositionSummary connected={connected} fhe={fhe} pools={pools} positions={positions} setModal={setModal} />
          )}
        </div>
      </div>
    </div>
...
function PortfolioView({ fhe, connected, setModal, pools, positions }: {
  fhe: boolean; connected: boolean;
  setModal: (m: ModalState | null) => void;
  pools: PoolView[]; positions: PositionRow[];
}) {
  if (!connected) {
    return (
  ...
  const supplied = positions.filter(p => BigInt(p.deposit_shares || "0") > 0n);
  const borrowed = positions.filter(p => BigInt(p.borrow_principal || "0") > 0n);
  const totalDeposits = supplied.reduce((s, p) => s + BigInt(p.deposit_shares || "0"), 0n);
  const totalBorrows = borrowed.reduce((s, p) => s + BigInt(p.borrow_principal || "0"), 0n);

  // Compute min health factor across positions
  const hfValues = positions
    .filter(p => p.health_factor_wad && BigInt(p.health_factor_wad) < (1n << 100n))
    .map(p => Number(BigInt(p.health_factor_wad!) * 100n / WAD) / 100);
  const minHF = hfValues.length > 0 ? Math.min(...hfValues) : null;
  const hfColor = minHF === null ? "#9ca3af" : minHF > 1.5 ? "#059669" : minHF > 1.1 ? "#d97706" : "#dc2626";
  ...
  return (
    <div className="fade-rise" style={{ maxWidth: 1240, margin: "0 auto", padding: "0 20px 40px" }}>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 10, marginBottom: 20 }}>
        ...
        <MetricCard label="Deposit Shares" value={formatBigAmount(totalDeposits)} color="#059669" />
        <MetricCard label="Borrow Principal" value={formatBigAmount(totalBorrows)} color="#dc2626" />
        <MetricCard label="Min Health Factor" value={minHF !== null ? minHF.toFixed(2) : "—"} color={hfColor} />
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 300px", gap: 16, alignItems: "start" }}>
        ...
        <div className="glass-card" style={{ borderRadius: 18, padding: 20 }}>
          ...
          {supplied.length === 0 ? (
            <div style={{ textAlign: "center", padding: "16px 0", color: "#9ca3af", fontSize: 13 }}>No deposits yet</div>
          ) : supplied.map((pos, i) => {
            const pool = pools.find(p => p.poolAddress.toBase58() === pos.pool_address);
            if (!pool) return null;
            const enc = fhe && getPoolType(pool.symbol) === "enc";
            return (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, padding: "11px 13px", background: "#f9f9fb", borderRadius: 12, marginBottom: 8, border: "1px solid #f0f0f3" }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
                    <span style={{ fontSize: 14, fontWeight: 600 }}>{pool.symbol}</span>
                    <span style={{ fontSize: 14, fontWeight: 600 }}>{enc ? <CipherVal seed={i + 10} mask="◉◉◉◉" /> : formatBigAmount(BigInt(pos.deposit_shares))}</span>
                  </div>
                  <div style={{ display: "flex", justifyContent: "space-between", marginTop: 2 }}>
                    <Tag type={getPoolType(pool.symbol)} />
                    <span style={{ fontSize: 12, color: "#059669", fontWeight: 500 }}>+{poolSupplyApy(pool).toFixed(1)}% APY</span>
                  </div>
                </div>
                <button className="btn-borrow" style={{ fontSize: 11.5, padding: "4px 10px", flexShrink: 0 }} onClick={() => setModal({ type: "withdraw", pool })}>
                  Withdraw
                </button>
              </div>
            );
          ...
          {pools.length > 0 && (
            <button className="btn-supply" style={{ width: "100%", padding: "9px", borderRadius: 10, fontSize: 13, marginTop: 8 }} onClick={() => setModal({ type: "supply", pool: pools[0] })}>
              + Supply more
            </button>
          )}
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <div className="glass-card" style={{ borderRadius: 18, padding: 20 }}>
            ...
            {borrowed.length === 0 ? (
              <div style={{ textAlign: "center", padding: "16px 0", color: "#9ca3af", fontSize: 13 }}>No borrows yet</div>
            ) : borrowed.map((pos, i) => {
              const pool = pools.find(p => p.poolAddress.toBase58() === pos.pool_address);
              if (!pool) return null;
              return (
                <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, padding: "11px 13px", background: "#f9f9fb", borderRadius: 12, marginBottom: 8, border: "1px solid #f0f0f3" }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
                      <span style={{ fontSize: 14, fontWeight: 600 }}>{pool.symbol}</span>
                      <span style={{ fontSize: 14, fontWeight: 600 }}>{formatBigAmount(BigInt(pos.borrow_principal))}</span>
                    </div>
                    <div style={{ display: "flex", justifyContent: "space-between", marginTop: 2 }}>
                      <Tag type={getPoolType(pool.symbol)} />
                      <span style={{ fontSize: 12, color: "#dc2626", fontWeight: 500 }}>{poolBorrowApy(pool).toFixed(1)}% APY</span>
                    </div>
                  </div>
                  <button className="btn-borrow" style={{ fontSize: 11.5, padding: "4px 10px", flexShrink: 0 }} onClick={() => setModal({ type: "repay", pool })}>
                    Repay
                  </button>
                </div>
              );
            ...
            {pools.length > 0 && (
              <button className="btn-borrow" style={{ width: "100%", padding: "9px", borderRadius: 10, fontSize: 13, marginTop: 8 }} onClick={() => setModal({ type: "borrow", pool: pools[0] })}>
                + Borrow more
              </button>
            )}
          </div>
          ...
          <div className="glass-card" style={{ borderRadius: 18, padding: 20 }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, marginBottom: 5 }}>
              ...
              <span style={{ fontWeight: 700, color: hfColor }}>{minHF !== null ? `${minHF.toFixed(2)} · ${minHF > 1.5 ? "Safe" : minHF > 1.1 ? "Watch" : "Danger"}` : "—"}</span>
            </div>
            <div className="hf-bar-wrap" style={{ margin: "0 0 4px" }}>
              <div className="hf-bar" style={{ width: minHF !== null ? `${Math.min(minHF / 3 * 100, 100)}%` : "0", background: hfColor }} />
            </div>
...
function FlashView({ connected, fhe, pools }: { connected: boolean; fhe: boolean; pools: PoolView[] }) {
    ...
    <div className="fade-rise" style={{ maxWidth: 1240, margin: "0 auto", padding: "0 20px 40px" }}>
      {pools.length === 0 ? (
        <div style={{ background: "white", border: "1px solid #e5e7eb", borderRadius: 14, padding: "48px 24px", textAlign: "center" }}>
          <div style={{ fontSize: 16, fontWeight: 700, marginBottom: 6 }}>No pools available</div>
          <div style={{ fontSize: 13, color: "#6b7280" }}>Pools must be initialized before flash loans are available.</div>
        </div>
      ) : (
            ...
            {pools.map((p, i) => {
              ...
              return (
                <div key={p.poolAddress.toBase58()} style={{ display: "flex", alignItems: "center", gap: 8, padding: "7px 0", borderBottom: i < pools.length - 1 ? "1px solid #f7f7f9" : "none" }}>
                  ...
                  <span style={{ fontSize: 13, color: "#5b5b66" }}>{formatBigAmount(avail > 0n ? avail : 0n)}</span>
                  <span style={{ fontSize: 11, color: util > 70 ? "#dc2626" : util > 50 ? "#d97706" : "#059669", fontWeight: 600, background: util > 70 ? "#fef2f2" : util > 50 ? "#fffbeb" : "#ecfdf5", padding: "2px 7px", borderRadius: 999 }}>
                    {util.toFixed(0)}% used
                  </span>
                </div>
              );
          ...
          <div className="glass-card" style={{ borderRadius: 18, padding: 24 }}>
            ...
            <div style={{ marginBottom: 12 }}>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
                {pools.map((p, i) => (
                  <button key={p.poolAddress.toBase58()} onClick={() => setSelectedIdx(i)} style={{ padding: "5px 12px", borderRadius: 999, border: "1px solid", borderColor: selectedIdx === i ? "#6d28d9" : "#e7e7ec", background: selectedIdx === i ? "#ede9fe" : "white", color: selectedIdx === i ? "#4c1d95" : "#5b5b66", fontSize: 13, fontWeight: 600, cursor: "pointer", transition: "all .15s" }}>
                    {p.symbol}
                  </button>
                ))}
              </div>
            ...
            <div style={{ marginBottom: 12 }}>
              ...
              {pool && (
                <div style={{ fontSize: 11.5, color: "#5b5b66", marginTop: 4 }}>
                  Available: {formatBigAmount(pool.totalDeposits - pool.totalBorrows > 0n ? pool.totalDeposits - pool.totalBorrows : 0n)} · Fee: {amount ? (parseFloat(amount) * feeRate).toFixed(4) : 0} {pool.symbol}
                </div>
              )}
            </div>
            {pool && (
            ...
            {fhe ? (
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                ...
                <button
                  disabled={!connected || !amount || !pool || ["building", "signing", "confirming"].includes(status)}
                  onClick={() => { if (pool) { reset(); flashExecute(pool, BigInt(Math.round(parseFloat(amount || "0") * 1e9))); } }}
...
function IkaSetupModal({ pool, setModal }: { pool: PoolView; setModal: (m: ModalState | null) => void }) {
    ...
    create: (
      <div>
        ...
        <div style={{ display: "grid", gap: 8, marginBottom: 18 }}>
          {[
            { icon: "🔑", t: "Distributed key generation", d: "DKG runs over gRPC with the Ika pre-alpha network" },
        ...
        <button disabled={busy || !publicKey} onClick={handleCreate}
          style={{ width: "100%", padding: "11px", borderRadius: 12, background: publicKey ? accentBg : "#e7e7ec", color: publicKey ? "white" : "#9ca3af", border: "none", fontSize: 14, fontWeight: 700, cursor: publicKey ? "pointer" : "not-allowed" }}>
    ...
    transfer: (
      <div>
        ...
        <button disabled={busy} onClick={handleTransfer}
          style={{ width: "100%", padding: "11px", borderRadius: 12, background: accentBg, color: "white", border: "none", fontSize: 14, fontWeight: 700, cursor: "pointer" }}>
...
function Modal({ modal, setModal, fhe }: { modal: ModalState; setModal: (m: ModalState | null) => void; fhe: boolean }) {
  ...
  const btnBg =
    poolType === "ika"   ? "linear-gradient(135deg,#f97316,#eab308)"
    : poolType === "enc" ? "linear-gradient(135deg,#6d28d9,#9333ea)"
    : poolType === "oro" ? "linear-gradient(135deg,#eab308,#ca8a04)"
    :                       "#0b0b10";
    ...
    <div className="modal-backdrop" onClick={(e) => { if (e.target === e.currentTarget) setModal(null); }}>
      <div style={{ background: "white", borderRadius: 20, padding: 24, width: "100%", maxWidth: 380, boxShadow: "0 30px 60px -12px rgba(76,29,149,.2),0 12px 30px -8px rgba(10,10,20,.15)", position: "relative" }}>
        ...
        <div style={{ fontSize: 11.5, color: "#5b5b66", marginBottom: 14 }}>
          Amount in base units (lamports) · {isSupply ? "Deposit" : "Borrow"} into {pool.symbol} pool
        </div>
        ...
        {isPrivate ? (
          ...
          >
            ...
            : status === "signing"    ? "Approve in wallet…"
            : status === "confirming" ? "Confirming…"
            : type === "supply"       ? `Supply${poolType === "ika" ? " via Ika dWallet" : ""}`
...
export default function DAppPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/positions/${encodeURIComponent(publicKey.toBase58())}`, { cache: "no-store" })
      ...
      .then((d: { positions: PositionRow[] }) => setPositions(d.positions ?? []))
      .catch(() => setPositions([]));
  }, [publicKey]);
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column" }}>
      ...
      <main style={{ flex: 1, paddingTop: 20, position: "relative" }}>
        {view === "markets"   && <MarketsView fhe={fhe} connected={connected} setModal={setModal} pools={pools} poolsLoading={poolsLoading} poolsError={poolsError} refreshPools={refreshPools} positions={positions} />}
        {view === "portfolio" && <PortfolioView fhe={fhe} connected={connected} setModal={setModal} pools={pools} positions={positions} />}
